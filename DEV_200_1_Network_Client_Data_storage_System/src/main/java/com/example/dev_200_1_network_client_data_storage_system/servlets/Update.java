...
import com.example.dev_200_1_network_client_data_storage_system.beans.ReturningErrors;
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (request.getParameter("UpdateDataClient") != null) {
            ...
            if (!updateBean.updateClient(strId, name, type)) {
                ...
                response.sendRedirect("http://localhost:8080/DEV_200_1_Network_Client_Data_storage_System-1.0-SNAPSHOT/Update?client-id=" + strId + "&name=UpdateClient");
        ...
        if (request.getParameter("UpdateClientAddress") != null) {
            ...
            if (!updateBean.updateAddress(strId, macAdr, newMac, ip, model, address)) {
                request.getSession().setAttribute("Error_field", returningErrors.getErrorFieldType());
                request.getSession().setAttribute("Error_reason", returningErrors.getErrorReasonType());
                response.sendRedirect("http://localhost:8080/DEV_200_1_Network_Client_Data_storage_System-1.0-SNAPSHOT/Update?client-id=" + strId + "&name=UpdateAddress&mac=" + macAdr);
            } else response.sendRedirect("ViewList");
        }
        if (request.getParameter("CreateClientAddress") != null) {
            ...
            if (!updateBean.createAddress(strId, ip, mac, model, address)) {
                request.getSession().setAttribute("Error_field", returningErrors.getErrorFieldType());
                request.getSession().setAttribute("Error_reason", returningErrors.getErrorReasonType());
                response.sendRedirect("http://localhost:8080/DEV_200_1_Network_Client_Data_storage_System-1.0-SNAPSHOT/Update?client-id=" + strId + "&name=CreateAddress");
            } else response.sendRedirect("ViewList");
        }
    }
}
