...
import com.example.dev_200_1_network_client_data_storage_system.models.ErrorFieldType;
import com.example.dev_200_1_network_client_data_storage_system.models.ErrorReasonType;
...
public class ValidatorBean {
    ...
    public boolean validateClientList(String id, String name, String type) throws ServletException, IOException {
        if (validateId(id, ErrorFieldType.CLIENT_ID.getType()) &&
                validateClient(name, 100, ErrorFieldType.CLIENT_NAME.getType(), true, false) &&
    ...
    public boolean validateAddress(String data, int maxLength, String errorField, boolean isIp, boolean isMac) throws IOException {
        ...
        if (validateIsEmpty(data, errorField) && validateLength(data, maxLength, errorField)) {
            if (isIp) {
                ...
                if (clientRepository.findByAddressMac(data)!=null){
                    returningErrors.setErrorFieldType(errorField);
    ...
    public boolean validateId(String data, String errorField) throws IOException {
        if (validateIsEmpty(data, errorField)) {
            try {
                Integer.parseInt(data.trim());
            } catch (NumberFormatException e) {
                returningErrors.setErrorFieldType(errorField);
                returningErrors.setErrorReasonType(ErrorReasonType.ERROR_ANINTEGER.getType());
                return false;
            }
        }
        return true;
